...
//   // ✅ ONLY CHANGE: Simple fetch with no extra headers/cache
//   useEffect(() => {
...
import { motion } from "framer-motion";
import Image from "next/image";
import { Star, Award, Users, ChevronRight, MessageCircle, Phone, Video, CheckCircle, Sparkles, Clock, ChevronLeft, Shield, Lock, Zap, Download, Mail, Quote, Play, Camera, Heart } from "lucide-react";
import AppModal from "@/components/ui/AppModal";
import Link from "next/link";
import useEmblaCarousel from 'embla-carousel-react';
import EnquiryModal from "../ui/EnquiryModal";
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 md:py-16 relative z-10">
          <div className="max-w-6xl mx-auto">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              ...
              {/* ✨ Right - Image with Unique Design */}
              <motion.div
                initial={{ opacity: 0, x: 30 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.6, delay: 0.2 }}
              ...
              >
                <div className="relative rounded-2xl overflow-hidden shadow-2xl shadow-amber-500/10">
                  ...
                  {/* ✨ Unique Gradient Overlay - Dual Tone */}
                  <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-amber-500/10 to-transparent" />
                  ...
                  <motion.div
                    ...
                    className="absolute top-4 left-4 bg-black/60 backdrop-blur-md rounded-2xl px-4 py-2 border border-amber-500/30"
                  >
                    <div className="flex items-center gap-2">
                      <Quote size={14} className="text-amber-400" />
                      <span className="text-white text-xs font-medium">Trusted by 10K+ Students</span>
                    </div>
                  </motion.div>

                  {/* ✨ Floating Video Badge - Top Right */}
                  <motion.div
                    initial={{ opacity: 0, scale: 0.8 }}
                    animate={{ opacity: 1, scale: 1 }}
                    transition={{ delay: 0.6, duration: 0.5 }}
                    className="absolute top-4 right-4 bg-amber-500/90 backdrop-blur-sm rounded-full px-3 py-1.5 flex items-center gap-1.5 shadow-lg shadow-amber-500/30"
                  >
                    <Play size={12} className="text-black" />
                    <span className="text-black text-xs font-bold">Live Session</span>
                  </motion.div>

                  {/* ✨ Animated Rating Ring - Left Side */}
                  <motion.div
                    initial={{ opacity: 0, x: -20 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: 0.5, duration: 0.5 }}
                    className="absolute top-1/2 -left-3 -translate-y-1/2 bg-white/95 backdrop-blur-sm rounded-2xl p-3 shadow-2xl border border-amber-200 hidden lg:block"
                  >
                    <div className="flex flex-col items-center">
                      ...
                      <div className="w-6 h-0.5 bg-amber-200 my-1" />
                      <span className="text-gray-500 text-[9px] font-medium">Top Rated</span>
                    </div>
                  </motion.div>

                  {/* ✨ Animated Experience Badge - Right Side */}
                  <motion.div
                    initial={{ opacity: 0, x: 20 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: 0.7, duration: 0.5 }}
                    className="absolute top-1/2 -right-3 -translate-y-1/2 bg-white/95 backdrop-blur-sm rounded-2xl p-3 shadow-2xl border border-amber-200 hidden lg:block"
                  >
                    <div className="flex flex-col items-center">
                      <Award size={20} className="text-amber-500" />
                      <span className="text-gray-900 font-bold text-sm">10+</span>
                      <span className="text-gray-500 text-[9px] font-medium">Years</span>
                    </div>
                  </motion.div>
                  ...
                  <div className="absolute bottom-4 left-4 right-4 flex flex-col gap-2">
                    {/* Main Badge */}
                    <div className="bg-white/95 backdrop-blur-sm rounded-xl p-3 border border-amber-200 shadow-lg">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 rounded-full bg-gradient-to-r from-amber-500 to-amber-600 flex items-center justify-center">
                            <Sparkles className="w-4 h-4 text-white" />
                    ...
                    {/* ✨ Extra Badge - "Book Now" with animation */}
                    <motion.div
                      animate={{ scale: [1, 1.02, 1] }}
                      transition={{ duration: 2, repeat: Infinity }}
                      className="bg-amber-500/90 backdrop-blur-sm rounded-xl px-3 py-1.5 border border-amber-300/50 shadow-lg flex items-center justify-between"
                    >
                      <div className="flex items-center gap-2">
                        <Camera size={12} className="text-black" />
                        <span className="text-black text-[10px] font-semibold">Book 1-on-1 Session</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <Heart size={10} className="text-black fill-black" />
                        <span className="text-black text-[9px] font-medium">98% Happy</span>
                      </div>
                    </motion.div>
                  </div>
                  ...
                  <div className="absolute top-0 right-0 w-20 h-20 overflow-hidden">
                    <div className="absolute top-0 right-0 w-12 h-12 bg-amber-500/10 -translate-y-1/2 translate-x-1/2 rotate-45" />
                  </div>
                  <div className="absolute bottom-0 left-0 w-20 h-20 overflow-hidden">
                    <div className="absolute bottom-0 left-0 w-12 h-12 bg-amber-500/10 translate-y-1/2 -translate-x-1/2 rotate-45" />
                  </div>
                </div>
              </motion.div>
            </div>
